...
async function findVertical(name, want) {
  await visit(name, new Function('want', `
    ...
    const c0 = [Math.floor(b.x / 4.0), Math.floor((b.y + .35) / 2.6), Math.floor(b.z / 4.0)];
    for (let r = 0; r < 16; r++)
      for (let dy = -2; dy <= 2; dy++) for (let dz = -r; dz <= r; dz++) for (let dx = -r; dx <= r; dx++) {
        ...
        return [(x + .5) * 4.0 + Math.sin(yaw) * 2.3, y * 2.6 + .1, (z + .5) * 4.0 + Math.cos(yaw) * 2.3, yaw, 0.16];
      }
    return null;`));
}
...
await visit('void', () => {
  const w = window.__wb.world, b = window.__wb.body;
  const c0 = [Math.floor(b.x / 4.0), Math.floor(b.z / 4.0)];
  for (let r = 0; r < 40; r++)
    for (let dz = -r; dz <= r; dz++) for (let dx = -r; dx <= r; dx++) {
      ...
      for (let y = 3; y > -22; y--) {
        ...
        return [(x + .5) * 4.0, y * 2.6 + 0.1, (z + .5) * 4.0, 0.9, 0.62];
      }
    }
  return null;
});
  ...
  for (let r = 0; r < 16; r++)
    for (let dy = -2; dy <= 2; dy++) for (let dz = -r; dz <= r; dz++) for (let dx = -r; dx <= r; dx++) {
      ...
      for (const [d, yaw] of [[0, -Math.PI / 2], [1, Math.PI / 2], [4, Math.PI], [5, 0]]) {
        ...
        return [(x + .5) * 4.0 - Math.sin(yaw) * 1.6, y * 2.6 + .1, (z + .5) * 4.0 - Math.cos(yaw) * 1.6, yaw, 0.05];
      }
    }
  return null;
});
...
await visit('shaft', () => {
  ...
  for (let r = 0; r < 14; r++)
    for (let dy = -2; dy <= 2; dy++) for (let dz = -r; dz <= r; dz++) for (let dx = -r; dx <= r; dx++) {
      ...
      return [(x + .5) * 4.0, (y + 1) * 2.6 + .1, (z + .5) * 4.0, 0.7, -0.72];
    }
  return null;
});
...
await visit('door', () => {
  ...
  return [(g.x + .5) * 4.0 + n[0] * 0.9, g.y * 2.6 + 0.1, (g.z + .5) * 4.0 + n[2] * 0.9, yaw, 0.0];
});
