...
try {
 ...
 await touch('touchStart',[[1,95,700]]);await touch('touchMove',[[1,95,676]]);
 assert((await state()).forward>.4);assert(!(await state()).dash);
 const walk=await page.evaluate(()=>{const w=__wb;w.look(0,0);w.sim(.12);return Math.hypot(w.body.vx,w.body.vz);});
 await touch('touchMove',[[1,95,652]]);assert((await state()).forward>.99);assert((await state()).dash);
 ...
 await touch('touchStart',[[1,95,652],[2,300,700]]);
 await touch('touchMove',[[1,143,700],[2,340,680]]);assert((await state()).strafe>.99);assert(!(await state()).dash);
 const facing=await page.evaluate(()=>{__wb.sim(1/60);return __wb.camera.parent.rotation.y;});assert(Math.abs(facing)>.1,'Right drag turns camera');
 await touch('touchEnd',[[2,340,680]]);assert((await state()).strafe>.99,'Left movement finger remains active');
 await touch('touchCancel',[]);assert.deepEqual(await state(),{forward:0,strafe:0,dash:false,jump:false});
 ...
 await touch('touchStart',[[4,95,700]]);await touch('touchMove',[[4,143,700]]);await page.evaluate(()=>dispatchEvent(new Event('blur')));assert(!(await state()).strafe);await touch('touchEnd',[]);
 ...
 await touch('touchStart',[[5,95,700]]);await touch('touchMove',[[5,95,652]]);await page.setViewportSize({width:844,height:390});await page.waitForTimeout(100);assert(!(await state()).dash);assert(!(await state()).forward);await touch('touchCancel',[]);
 ...
 assert.deepEqual(errors,[]);console.log('PASS: mobile start without pointer lock, analog forward/strafe, full-forward dash, simultaneous movement/right-look, independent release, cancel, jump, blur, pause/resume, rotation reset, portrait/landscape, desktop controls; no page errors.');
