...
export function createTouchControls({ look, mark, pause }) {
  ...
  root.innerHTML = `<div id="touch-stick" aria-label="移動"><div id="stick-ring"><div id="stick-knob"></div></div><span>移動</span></div>
    <div id="touch-dash" aria-label="視点操作"><span>ドラッグで視点</span></div>
  ...
  const state = { forward: 0, strafe: 0, dash: false, jump: false };
  ...
  if (enabled) {
    ...
    help.textContent = '左スティック：前後左右に移動。前へ大きく倒すとダッシュ。右側：ドラッグで視線。';
    document.querySelector('#keys').after(help);
  }
  function reset() {
    state.forward = state.strafe = 0; state.dash = state.jump = false;
  ...
  bind(el('touch-stick'), e => { originX = e.clientX; originY = e.clientY; }, e => {
    ...
    state.strafe = dead(x); state.forward = -dead(y);
    state.dash = state.forward > .82 && length > .9;
    el('stick-knob').style.transform = `translate(${x * 48}px,${y * 48}px)`;
  }, () => { state.forward = state.strafe = 0; state.dash = false; el('stick-knob').style.transform = ''; });
  let previousX = 0, previousY = 0;
  bind(el('touch-dash'), e => {
    previousX = e.clientX; previousY = e.clientY; el('touch-dash').classList.add('active');
  }, e => {
    look((e.clientX - previousX) * .004, (e.clientY - previousY) * .004);
    previousX = e.clientX; previousY = e.clientY;
  }, () => { el('touch-dash').classList.remove('active'); });
